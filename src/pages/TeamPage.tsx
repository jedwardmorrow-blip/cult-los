...
export default function TeamPage() {
  ...
  async function fetchTeam() {
    ...
    const memberData = await Promise.all(profiles.map(async profile => {
  ...
  const selectedMember = members.find(m => m.profile.id === selected)
  const getInitials = (name: string) => name.split(' ').map((n: string) => n[0]).join('').toUpperCase()
  const getProgress = (goals: Goal[]) => !goals.length ? 0 : Math.round((goals.filter(g => g.status === 'completed').length / goals.length) * 100)
      ...
      <div className="grid grid-cols-3 gap-6">
        <div className="space-y-2">
          {members.map(({ profile, goals }) => {
            ...
            return (
              <button key={profile.id} onClick={() => setSelected(profile.id)} className={`w-full text-left p-3 rounded-lg border transition-all ${isSelected ? 'bg-cult-gold/10 border-cult-gold/30' : 'bg-cult-card border-cult-border hover:border-cult-muted'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 font-mono text-xs font-medium ${isSelected ? 'bg-cult-gold/20 border border-cult-gold/40 text-cult-gold' : 'bg-cult-muted border border-cult-border text-cult-text'}`}>
                ...
                {goals.length > 0 && (
                  <div className="mt-2">
                    <div className="h-1 bg-cult-muted rounded-full overflow-hidden">
                      <div className="h-full bg-cult-gold/60 rounded-full" style={{ width: `${progress}%` }} />
                    </div>
                    <div className="font-mono text-[9px] text-cult-text mt-1">{progress}% complete</div>
                  </div>
                )}
              </button>
            )
            ...
            <div className="card p-6 space-y-6">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-full bg-cult-gold/20 border border-cult-gold/30 flex items-center justify-center flex-shrink-0">
                  <span className="font-mono text-sm text-cult-gold font-medium">{getInitials(selectedMember.profile.full_name)}</span>
                </div>
                <div>
                  <h2 className="font-display text-2xl tracking-wider text-cult-white">{selectedMember.profile.full_name.toUpperCase()}</h2>
                  <div className="font-mono text-xs text-cult-text">{ROLE_LABELS[selectedMember.profile.role] || selectedMember.profile.role}</div>
                </div>
              </div>

              {selectedMember.northStar && (
                <div className="bg-cult-gold/5 border border-cult-gold/20 rounded-lg p-4">
                  <div className="font-mono text-[10px] tracking-[0.3em] text-cult-gold/60 uppercase mb-2 flex items-center gap-1.5">
                    <Target size={10} /> North Star
                  </div>
                  <p className="text-cult-white text-xs leading-relaxed">{selectedMember.northStar}</p>
                </div>
              )}

              {[1, 2, 3].map(phase => {
                const phaseGoals = selectedMember.goals.filter(g => g.phase === phase)
                if (!phaseGoals.length) return null
                const phaseLabels: Record<number, string> = { 1: 'Days 1–30', 2: 'Days 31–60', 3: 'Days 61–90' }
                const phaseColors: Record<number, string> = { 1: 'text-cult-gold', 2: 'text-blue-300', 3: 'text-purple-300' }
                return (
                  <div key={phase}>
                    <div className={`font-mono text-[10px] tracking-[0.3em] uppercase mb-2 ${phaseColors[phase]}`}>{phaseLabels[phase]}</div>
                    <div className="space-y-2">
                      {phaseGoals.map(goal => {
                        const s = STATUS_ICONS[goal.status]
                        const StatusIcon = s.icon
                        return (
                          <div key={goal.id} className="flex items-start gap-2.5">
                            <StatusIcon size={13} className={`${s.cls} mt-0.5 flex-shrink-0`} />
              ...
              {selectedMember.goals.length === 0 && (
                <div className="text-center py-8">
                  <Target size={24} className="text-cult-text mx-auto mb-2" />
                  <p className="text-cult-text text-xs font-mono">No plan goals yet</p>
                </div>
              )}
            </div>
          ) : (
            <div className="card p-6 flex items-center justify-center h-64">
              <p className="text-cult-text text-xs font-mono">Select a team member</p>
            </div>
          )}
        </div>
      </div>
